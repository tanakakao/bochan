"""Record raw and cost-aware MFKG scores at each allowed fidelity."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--fixed-cost", type=float, default=0.25)
    parser.add_argument("--budget", type=float, default=15.0)
    parser.add_argument("--n-initial", type=int, default=6)
    parser.add_argument("--max-steps", type=int, default=3)
    parser.add_argument("--num-restarts", type=int, default=2)
    parser.add_argument("--raw-samples", type=int, default=32)
    parser.add_argument("--fit-maxiter", type=int, default=30)
    parser.add_argument("--num-fantasies", type=int, default=8)
    parser.add_argument("--output", type=Path, default=Path("mfkg_fidelity_scores.csv"))
    return parser
...
def main() -> None:
    ...
    for seed in args.seeds:
        result = run_mfkg_fidelity_score_diagnostic(problem, seed=seed, config=config)
        ...
        print(
            problem.name,
            f"seed={seed}",
            f"final_metric={result.run.metric[-1].item():.6g}",
        )
