...
def prepare_binary_multiclass_codemod() -> None:
...
'''
    text = replace_once(text, old, new, label="refine multiclass observed init codemod")
    text = replace_once(
        text,
        "    patch_ordinal()\n",
        "",
        label="remove ordinal patch",
    )
    SCRIPT.write_text(text, encoding="utf-8")


def patch_multiclass_active_learning_public_constructor() -> None:
    path = ROOT / "src/bochan/acquisition/multiclass/active_learning/single_output.py"
    ...
    text = replace_once(
        ...
        "        X_observed: Tensor | None = None,\n",
        label="multiclass active-learning public duplicate parameters",
    )
    text = replace_once(
        text,
        ...
        "            X_pending=X_pending,\n"
        "            X_observed=X_observed,\n",
        label="multiclass active-learning public duplicate forwarding",
    )
    path.write_text(text, encoding="utf-8")


def write_tests() -> None:
    legacy_path = ROOT / "tests/test_classification_duplicate_controls_e2e.py"
    legacy_path.unlink(missing_ok=True)

    path = ROOT / "tests/test_binary_multiclass_duplicate_controls_e2e.py"
    path.write_text(
        '''from __future__ import annotations
...
def test_binary_and_multiclass_resolve_observed_consistently() -> None:
    ...
    assert torch.isinf(
        binary._observed_penalty_per_point(
            binary_model.train_X[:1].view(1, 1, 1)
        )
    ).all()
...
def main() -> None:
    prepare_binary_multiclass_codemod()
    runpy.run_path(str(SCRIPT), run_name="__main__")
    patch_multiclass_active_learning_public_constructor()
    write_tests()
