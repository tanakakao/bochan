"""InputPerturbation compatibility for multiclass hypervolume acquisitions.
...
def infer_input_perturbation_n_w(model: Any) -> Optional[int]:
    ...
    tensor instead of a concrete BoTorch class check, so chained and compatible
...
def patch_multiclass_hypervolume_input_perturbation(
    ...
    *,
    ...
    @wraps(original_init)
    def compatible_init(self, model, *args, **kwargs):
    ...
    def compatible_compute_qehvi(self, samples: Tensor, X: Optional[Tensor] = None):
    ...
    acquisition_cls._compute_qehvi = compatible_compute_qehvi
