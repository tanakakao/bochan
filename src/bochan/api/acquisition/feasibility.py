"""Outcome-constraint composition for high-level acquisition construction."""
...
def _constraint_specs(config: Any) -> list[Any]:
    ...
    for output, operator, threshold in zip(
        config.output_indices, config.operators, config.thresholds, strict=True
    ):
        sense = "ge" if str(operator).lower() in {"ge", "gt"} else "le"
        specs.append(
            FeasibilityConstraintSpec(
                output=int(output), threshold=float(threshold), sense=sense
            )
        )
    return specs
...
def prepare_feasibility_build(*, bundle: Any, config: Any) -> tuple[Any, FeasibilityBuildPlan | None]:
    ...
    return base_config, FeasibilityBuildPlan(
        constraints=tuple(wrapper_constraints), config=constraint_config
    )
...
def build_outcome_constrained_acquisition(
    *, bundle: Any, config: Any, data_context: Any | None = None
) -> Any:
    ...
    acqf = build_acquisition(
        bundle=bundle, config=base_config, data_context=data_context
    )
    return apply_feasibility_build_plan(
        acqf=acqf, model=bundle.model, plan=plan
    )
