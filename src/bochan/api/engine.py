...
def _filter_context_fields_for_acqf(config: AcquisitionConfig) -> AcquisitionConfig:
    ...
    Some bochan acquisition classes accept ``**kwargs`` and forward them to
    BoTorch / GPyTorch base classes. Passing automatic context fields such as
    ``X_baseline`` to those classes can therefore fail with errors like
    ``MCAcquisitionFunction.__init__() got an unexpected keyword argument``.

    Explicit ``acqf_kwargs`` are preserved. This helper only filters the
