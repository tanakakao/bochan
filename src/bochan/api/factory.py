    ...
    *,
    ...
    return ModelBundle(
        ...
        metadata={"model_cls": model_cls_name, "model_factory": config.model_factory is not None,
                  "latent_posterioramily": "non_gaussian" if _is_non_gaussian_regression_model(model) else "gaussian",
                  "multi_output": False,
...
def build_multi_output_model(train_X: Any, train_Y: Any, config: ModelConfig, *, model_registry: Mapping[Any, Any] | None = None) -> ModelBundle:
    ...
    return ModelBundle(
        ...
        metadata={
            ...
            "latent_posterioramily": "non_gaussian" if all(_is_non_gaussian_regression_model(b.model) for b in sub_bundles) else ("hybrid" if any(_is_non_gaussian_regression_model(b.model) for b in sub_bundles) else "gaussian"),
