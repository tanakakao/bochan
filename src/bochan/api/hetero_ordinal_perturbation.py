"""InputPerturbation support for heteroscedastic ordinal objectives."""
...
def _patch_utility_forward() -> None:
    ...
    def supported_forward(self, samples: Tensor, X: Tensor | None = None) -> Tensor:
        if X is None:
            raise ValueError(
...
def _patch_hetero_nparego_wrapper() -> None:
    ...
    @wraps(original)
    def supported(*args: Any, objective=None, **kwargs: Any):
    ...
    supported._bochan_input_perturbation_patched = True
    supported._bochan_original = original
    package.qHeteroMultiOutputOrdinalNParEGO = supported


def apply_hetero_ordinal_perturbation() -> None:
    """Install hetero ordinal InputPerturbation support once."""
...
__all__ = ["apply_hetero_ordinal_perturbation"]
