"""Fusion modules for material representations and process features."""
...
class ConcatFusion(MaterialProcessFusion):
    ...
    Args:
        material_dim: Expected width of ``material_features``.
        process_dim: Expected width of ``process_features``. Set this to zero
            for composition-only models, where ``process_features`` may be
            omitted.
    """
...
def build_material_process_fusion(
    ...
    *,
    ...
    Passing a module directly supports custom fusion strategies without adding
    strategy-specific branches to material encoders or DeepKernel models.
    """
