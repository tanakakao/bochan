...
from .support import ScoreMode, SupportSelection, sample_k_without_replacement, select_support_mask
...
def make_k_sparse_linear_constraints_repair(
    ...
    *,
    ...
    Args:
        ...
        support_selection: Local support-selection strategy. ``"best_subset"``
            is acquisition-aware and must be resolved before this repair layer.
    ...
    def repair(X: Tensor) -> Tensor:
        ...
        if comp_idx:
            ...
            group = Xf[:, idx_t]
            support_mask = select_support_mask(
                group,
                k=k,
                score=score,
