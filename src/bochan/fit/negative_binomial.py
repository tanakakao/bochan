...
def fit_negative_binomial_mll(
    ...
    if not hasattr(latent_model, "train_inputs") or not hasattr(latent_model, "train_targets"):
        raise AttributeError(
            ...
            "Create mll via make_negative_binomial_mll(model) or "
            "VariationalELBO(..., model=model.model, ...)."
        )
    ...
    for epoch in range(num_epochs):
        ...
        if verbose and ((epoch == 0) or ((epoch + 1) % 20 == 0)):
            probs = float(torch.clamp(likelihood.probs.detach(), 1e-6, 1 - 1e-6).mean().cpu())
            print(
                f"[NegativeBinomialGP] epoch={epoch+1:4d} "
