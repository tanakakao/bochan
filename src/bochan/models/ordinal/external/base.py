...
def _require_ordinal_targets(
    ...
    *,
    ...
    """Validate ordinal labels encoded as consecutive integers from zero."""
...
def _threshold_targets(labels: np.ndarray, num_classes: int) -> np.ndarray:
    """Return K-1 cumulative binary targets ``1[y > threshold]``."""
...
def _pava_nonincreasing_row(values: np.ndarray) -> np.ndarray:
    """Project one vector onto the non-increasing cone with unit weights."""
...
def _project_nonincreasing_probabilities(values: np.ndarray) -> np.ndarray:
    """Apply PAVA row-wise to cumulative exceedance probabilities."""
...
def _class_probs_from_cumulative(
    ...
    *,
    ...
    """Convert ``P(y > k)`` values to a valid ordinal class distribution."""
    ...
    probs = probs / np.clip(denom, eps, None)
    return probs


def _cumulative_from_class_probs(probabilities: Tensor) -> Tensor:
    """Return cumulative exceedance probabilities from class probabilities."""
...
class _ExternalCumulativeOrdinalMixin(_ExternalClassifierMixin):
    ...
    def make_mll(self, **kwargs: Any) -> None:
        """Signal that this external ordinal model is fitted directly."""
        del kwargs
        return None

    @property
    def ordinal_likelihood(self) -> OrdinalLogitLikelihood:
        """Compatibility likelihood used by existing ordinal acquisitions."""
    ...
    def ordinal_probability_posterior(
        ...
        **kwargs: Any,
        ...
        """Return the exact finite class-probability ensemble."""
        del kwargs
        self.eval()
    ...
    def probability_posterior(
        ...
        **kwargs: Any,
        ...
        """Alias for the exact ordinal class-probability posterior."""
    ...
    def forward(self, X: Tensor) -> MultivariateNormal:
        """Return the proportional-odds latent Gaussian compatibility bridge."""
    ...
    def latent_posterior(self, X: Tensor, **kwargs: Any) -> GPyTorchPosterior:
        """Alias used by existing ordinal active-learning acquisitions."""
        return self.posterior(X, **kwargs)

    def class_probs(self, X: Tensor) -> Tensor:
        """Return exact mean ordinal class probabilities."""
    ...
    def expected_utility(self, X: Tensor, utilities: Tensor) -> Tensor:
        """Return exact expected utility from the cumulative class distribution."""
...
def _initialize_external_ordinal_model(
    ...
    *,
    ...
    """Initialize shared tensors and fixed ordinal bridge likelihood."""
