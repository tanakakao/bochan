...
from bochan.models.classification.external.base import _align_probability_columns
...
def _new_random_forest_classifier(kwargs: Mapping[str, Any]) -> Any:
    """Create a binary RandomForestClassifier lazily."""
    try:
...
def _positive_probability(estimator: Any, X: np.ndarray, *, model_name: str) -> np.ndarray:
    """Return aligned class-1 probability from a binary estimator."""
...
class RandomForestOrdinalModel(_ExternalCumulativeOrdinalMixin, Model):
    ...
    Each threshold forest models ``P(y > k)``. Corresponding tree indices across
    threshold forests are paired into finite ordinal probability members, so
    tree disagreement provides a practical epistemic uncertainty estimate.
    """
    ...
    def fit(
        ...
        **ignore: Any,
        ...
        """Fit one binary forest for every cumulative ordinal threshold."""
        del ignore
