...
class _WideBetaMultiTaskCore(ApproximateGPyTorchModel):
...
class WideBetaMultiTaskGPModel(_WideBetaMultiTaskCore):
    """Correlated wide Beta model that omits, rather than imputes, NaN cells."""
...
class BetaMultiTaskGPModel(_WideBetaMultiTaskCore):
...
class KroneckerMultiTaskBetaGPModel(WideBetaMultiTaskGPModel):
    """Separable variational Beta GP for a complete block design.
    ...
    def __init__(self, train_X: Tensor, train_Y: Tensor, **kwargs: Any) -> None:
        ...
        train_X = torch.as_tensor(train_X)
        train_Y = torch.as_tensor(train_Y, device=train_X.device, dtype=train_X.dtype)
        validate_complete_block(train_X, train_Y, family="Beta")
        super().__init__(train_X, train_Y, **kwargs)
