...
from bochan.models.regression.beta._components import (
    BetaMeanLink,
...
from bochan.models.regression.beta import BetaGPModel, BetaLogLikelihood, BetaMixedGPModel
...
class _BaseProjectedBetaModel(Model):
    """PCA / REMBO Beta wrapper の共通基底。"""
    ...
    def predict_beta_params(self, X: Tensor):
        return self.base_model.predict_beta_params(self.transform_inputs(X))

    def make_mll(self):
        return self.base_model.make_mll()


class _ContinuousProjectedBetaModel(_BaseProjectedBetaModel):
    ...
    def condition_on_observations(self, X: Tensor, Y: Tensor, **kwargs: Any):
        if kwargs.get("noise") is not None:
            raise NotImplementedError("noise is not supported for projected Beta models.")
        X = torch.as_tensor(X, device=self._raw_train_X.device, dtype=self._raw_train_X.dtype)
        if X.ndim == 1:
            X = X.unsqueeze(0)
...
class PCABetaGPModel(_ContinuousProjectedBetaModel):
    """PCA 射影後の低次元空間で学習する Beta GP。"""
...
class REMBOBetaGPModel(_ContinuousProjectedBetaModel):
    """REMBO 射影後の低次元空間で学習する Beta GP。"""
...
class _MixedProjectedBetaModel(_BaseProjectedBetaModel):
...
class PCABetaMixedGPModel(_MixedProjectedBetaModel):
    """連続列だけ PCA 射影し、カテゴリ列を保持する mixed Beta GP。"""
    ...
    def __init__(
        ...
        *,
        ...
        self.eps = float(eps)
        self.clip_targets = bool(clip_targets)
        self.base_model = BetaMixedGPModel(
            train_X=projected_X,
            train_Y=train_Y,
...
class REMBOBetaMixedGPModel(PCABetaMixedGPModel):
    """連続列だけ REMBO 射影し、カテゴリ列を保持する mixed Beta GP。"""
