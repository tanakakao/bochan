...
from bochan.models.regression.count.negative_binomial import (
...
def _fit_variational_nb_mll(
    model: NegativeBinomialGPModel | NegativeBinomialMixedGPModel,
    *,
    lr: float = 0.01,
    num_epochs: int = 300,
    ...
    """補助 Negative Binomial モデルを簡易 training loop で fit する。"""
    mll = model.make_mll()
    mll.train()
...
def _estimate_nb_noise_targets(
    model: NegativeBinomialGPModel | NegativeBinomialMixedGPModel,
    train_X: Tensor,
    train_Y: Tensor,
    *,
    ...
    """NB mean 予測と observed count の残差から追加分散 target を作る。"""
    with torch.no_grad():
        mean = model.predict_mean(train_X)
        if mean.ndim > 1 and mean.shape[-1] == 1:
            mean = mean.squeeze(-1)
        y = prepare_count_targets(train_Y, train_X).to(mean)
