...
from bochan.models.regression.count.negative_binomial._components import NBLink, NegativeBinomialLogLikelihood, align_like, prepare_count_targets
from bochan.models.regression.count.negative_binomial import NegativeBinomialGPModel, NegativeBinomialMixedGPModel


class SparseOutlierNegativeBinomialLikelihood(NegativeBinomialLogLikelihood, RelevancePursuitMixin):
    """
    学習点ごとの sparse mean-link offset を持つ Negative Binomial likelihood。

    学習時には latent value f_i に sparse offset δ_i を加えます。

        y_i ~ NB(mean=mean(f_i + δ_i), total_count=r)
    ...
    def __init__(
        ...
        *,
        ...
        NegativeBinomialLogLikelihood.__init__(
            self,
            link=link,
...
class RobustRelevancePursuitNegativeBinomialGPModel(NegativeBinomialGPModel):
    ...
    def __init__(
        ...
        *,
        ...
        likelihood = SparseOutlierNegativeBinomialLikelihood(
            dim=train_X.shape[-2],
