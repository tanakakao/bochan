...
from bochan.models.regression.count.poisson._components import clone_input_transform, prepare_count_targets
...
class _AlignedPoissonMixin:
    """Align non-Gaussian Poisson wrappers with regression / ordinal conventions."""
...
class PoissonGPModel(_AlignedPoissonMixin, _PoissonGPModel):
    """Poisson GP with transformed `train_inputs` and raw `train_inputs_raw`."""


class PoissonMixedGPModel(_AlignedPoissonMixin, _PoissonMixedGPModel):
    """Mixed-input Poisson GP aligned with the common wrapper API."""
...
__all__ = ["PoissonGPModel", "PoissonMixedGPModel"]
