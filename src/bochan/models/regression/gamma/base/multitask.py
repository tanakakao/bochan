"""Correlated variational multi-task Gamma regression models."""
...
from bochan.models.regression.gamma._components import GammaLink, GammaLogLikelihood
from bochan.models.transforms.outcome import PositiveScaleOutcomeTransform

from .models import (
    clone_input_transform,
    clone_outcome_transform,
    to_device_dtype_outcome_transform,
    to_device_dtype_transform,
)


class GammaMultiTaskPosterior(Posterior):
    """Gamma response posterior that preserves the joint task covariance."""
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            likelihood: Fitted Gamma likelihood.
            q: Number of candidate points.
            num_tasks: Number of tasks.
            add_observation_noise: Whether to include Gamma observation variance.
        """
        super().__init__()
    ...
    def mean(self) -> Tensor:
        """Return positive Gamma means with shape ``[..., q, m]``."""
    ...
    def variance(self) -> Tensor:
        ...
        return latent + self.mean.square() / concentration.clamp_min(
            self.likelihood.min_concentration
        )
    ...
    def rsample_from_base_samples(
        ...
        """Map common-random-number latent samples to Gamma response means."""
        latent = self.latent_posterior.rsample_from_base_samples(
            sample_shape=sample_shape,
            base_samples=base_samples,
        )
...
@GetSampler.register(GammaMultiTaskPosterior)
def _get_gamma_multitask_sampler(
    posterior: GammaMultiTaskPosterior,
    sample_shape: torch.Size,
    seed: int | None = None,
...
class _LatentGammaMultiTaskSVGP(ApproximateGP):
...
def _wide_to_long(train_X: Tensor, train_Y: Tensor) -> tuple[Tensor, Tensor, Tensor]:
    ...
    if not observed.any(dim=0).all():
        ...
        raise ValueError(f"Each Gamma task requires an observation; empty tasks: {missing}.")
...
class _WideGammaMultiTaskCore(ApproximateGPyTorchModel):
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize a correlated Gamma multi-task model.
        ...
        Args:
            ...
            outcome_transform: Positive-scale outcome transform.
            mean_module: Optional latent mean.
            covar_module: Optional data covariance.
            task_covar_module: Optional task covariance.
            link: Positive Gamma mean link.
            init_concentration: Initial Gamma concentration.
        ...
        outcome_transform = to_device_dtype_outcome_transform(
            clone_outcome_transform(outcome_transform), train_X
        )
        transformed_Y = train_Y.clone()
        if outcome_transform is not None:
            if not finite.all():
                if not isinstance(outcome_transform, PositiveScaleOutcomeTransform):
                    raise ValueError(
                        "Partially observed Gamma multitask targets require "
                        "PositiveScaleOutcomeTransform."
                    )
                scales = []
                for task in range(self.num_tasks):
                    values = train_Y[finite[:, task], task]
                    method = outcome_transform.scale_method
                    if method == "median":
                        ...
                        scale = torch.quantile(values.abs(), outcome_transform.quantile)
                    else:
                        scale = outcome_transform.scale[min(task, outcome_transform.scale.numel() - 1)]
                    scales.append(scale.clamp_min(outcome_transform.min_scale))
                outcome_transform.scale = torch.stack(scales)
                outcome_transform._is_fitted.fill_(True)
                transformed_Y[finite] = (train_Y / outcome_transform.scale)[finite]
            else:
                transformed_Y, _ = outcome_transform(train_Y, X=train_X)

        long_X, long_Y, observed = _wide_to_long(transformed_X, transformed_Y)
        likelihood = likelihood or GammaLogLikelihood(
            link=link, init_concentration=init_concentration, min_mean=min_mean
        )
        latent_model = _LatentGammaMultiTaskSVGP(
            long_X,
            long_Y,
    ...
    def posterior(
        ...
        **kwargs: Any,
        ...
        if output_indices is not None:
            raise NotImplementedError("Gamma multitask output subsetting is not implemented.")
        if torch.is_tensor(observation_noise):
            raise NotImplementedError("Tensor observation_noise is not supported.")
        posterior: Posterior = GammaMultiTaskPosterior(
    ...
    def condition_on_observations(self, X: Tensor, Y: Tensor, **kwargs: Any) -> GammaMultiTaskGPModel:
        """Rebuild the same correlated structure with additional raw observations."""
        if kwargs.get("noise") is not None:
            raise NotImplementedError("Gamma multitask conditioning does not accept noise.")
    ...
    def fantasize(self, X: Tensor, sampler: Any, **kwargs: Any) -> GammaMultiTaskGPModel:
        ...
        if fantasies.ndim != 2:
            raise NotImplementedError(
                "Batched Gamma multitask fantasies require batched variational training data. "
                "Use posterior samples directly for MC acquisition functions."
            )
        return self.condition_on_observations(X, fantasies, **kwargs)


class WideGammaMultiTaskGPModel(_WideGammaMultiTaskCore):
    """Correlated wide Gamma model that omits, rather than imputes, NaN cells."""
...
class GammaMultiTaskGPModel(_WideGammaMultiTaskCore):
...
class KroneckerMultiTaskGammaGPModel(WideGammaMultiTaskGPModel):
    """Separable variational Gamma GP for a complete block design.
    ...
    def __init__(self, train_X: Tensor, train_Y: Tensor, **kwargs: Any) -> None:
        ...
        train_X = torch.as_tensor(train_X)
        train_Y = torch.as_tensor(train_Y, device=train_X.device, dtype=train_X.dtype)
        validate_complete_block(train_X, train_Y, family="Gamma")
        super().__init__(train_X, train_Y, **kwargs)
...
__all__ = [
    ...
    "GammaMultiTaskPosterior",
    "WideGammaMultiTaskGPModel",
    "KroneckerMultiTaskGammaGPModel",
]
