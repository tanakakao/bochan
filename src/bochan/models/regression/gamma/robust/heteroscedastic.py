...
from bochan.models.regression.gamma import (
    GammaGPModel,
    GammaLogLikelihood,
    GammaMixedGPModel,
)
from bochan.models.regression.gamma.base.models import clone_outcome_transform
...
def _fit_variational_gamma_mll(
    model: GammaGPModel | GammaMixedGPModel,
    *,
    lr: float = 0.01,
    num_epochs: int = 300,
    ...
    """補助 Gamma モデルを簡易 training loop で fit する。"""
    mll = model.make_mll()
    mll.train()
...
def _estimate_gamma_noise_targets(
    model: GammaGPModel | GammaMixedGPModel,
    train_X: Tensor,
    train_Y: Tensor,
    *,
    ...
    """Gamma mean 予測と observed positive value の残差から追加分散 target を作る。"""
    with torch.no_grad():
        mean = model.predict_mean(train_X)
        if mean.ndim > 1 and mean.shape[-1] == 1:
            mean = mean.squeeze(-1)
        y = prepare_positive_targets(train_Y, train_X).to(mean)
        extra = (y - mean).pow(2).clamp_min(float(min_noise))
    return ensure_2d_col(extra)


class _HeteroscedasticGammaMixin:
    """Gamma heteroscedastic model 用 mixin。"""
    ...
    def posterior(
        ...
        **kwargs: Any,
        ...
        posterior = HeteroscedasticGammaPosterior(base_posterior=base_post, extra_noise_var=extra_noise)
        if posterior_transform is not None:
            posterior = posterior_transform(posterior)
        return posterior


class HeteroscedasticGammaGPModel(_HeteroscedasticGammaMixin, GammaGPModel):
    """追加分散 GP を持つ Gamma 回帰モデル。"""
    ...
    def __init__(
        ...
        *,
        ...
        if train_Yvar is None:
            ...
            noise_targets = _estimate_gamma_noise_targets(aux_model, train_X, train_Y, min_noise=min_noise)
        else:
...
class HeteroscedasticGammaMixedGPModel(_HeteroscedasticGammaMixin, GammaMixedGPModel):
    """mixed 入力版の heteroscedastic Gamma 回帰モデル。"""
