"""Correlated multi-output Gaussian processes over shared M3GNet representations."""
...
class _CorrelatedM3GNetMultiTaskMixin:
    """Expose shared diagnostics and posterior semantics for correlated M3GNet tasks."""
    ...
    def m3gnet_feature_extractor(self) -> _M3GNetGPFeatureExtractor:
        """Return the shared M3GNet structure/process feature extractor."""

        return cast(_M3GNetGPFeatureExtractor, self.deepkernel.feature_extractor)

    @property
    def material_encoder(self) -> M3GNetEncoder:
        """Return the single M3GNet encoder shared by every output task."""
...
def _validate_multitask_targets(
    ...
    *,
    ...
    """Validate the wide-target contract used by correlated M3GNet models."""
...
class M3GNetMultiTaskGPModel(
    _CorrelatedM3GNetMultiTaskMixin,
    DeepKernelGaussianGPModel,
):
    """Correlated multi-output GP with one shared frozen M3GNet encoder."""
    ...
    def _apply(
        ...
        return cast(M3GNetMultiTaskGPModel, module)


class M3GNetMultiTaskDKLModel(M3GNetMultiTaskGPModel):
    """Correlated M3GNet multitask DKL with shared encoder fine-tuning."""
...
class M3GNetMixedMultiTaskGPModel(
    _CorrelatedM3GNetMultiTaskMixin,
    DeepKernelGaussianMixedGPModel,
):
    """Correlated M3GNet multitask GP with categorical process variables."""
    ...
    def __init__(
        ...
        *,
        ...
        if not normalized_cat_dims:
            raise ValueError(
                "M3GNetMixedMultiTaskGPModel requires at least one categorical "
                "process dimension."
            )
    ...
    def _apply(
        ...
        if reference is not None:
            self._model_dtype = reference.dtype
            self._model_device = reference.device
        return cast(M3GNetMixedMultiTaskGPModel, module)
...
class M3GNetMixedMultiTaskDKLModel(M3GNetMixedMultiTaskGPModel):
    """Correlated mixed M3GNet multitask DKL with shared encoder fine-tuning."""
