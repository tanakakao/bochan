"""Fast crystal-structure relaxation with ALIGNN-FF and ASE."""
...
from .alignn_ff_residual import _DEFAULT_MODEL_NAME, _load_alignn_ff_calculator
...
def _structure_mapping(atoms: Any) -> dict[str, Any]:
    return {
...
def _force_rows(forces: Any) -> tuple[tuple[float, float, float], ...]:
    ...
    if any(len(row) != 3 for row in rows):
        raise ValueError("ALIGNN-FF forces must have shape [n_atoms, 3].")
    return rows
...
def _stress_rows(stress: Any) -> tuple[
    ...
    if len(rows) != 3 or any(len(row) != 3 for row in rows):
        raise ValueError("ALIGNN-FF stress must have shape [3, 3].")
...
def _max_force_norm(forces: tuple[tuple[float, float, float], ...]) -> float:
    if not forces:
        return 0.0
    return max(sqrt(sum(component * component for component in row)) for row in forces)


class ALIGNNFFStructureRelaxer:
    """Relax periodic structures through the ALIGNN-FF ASE calculator."""
    ...
    def relax(
        ...
        *,
        ...
        dynamics = _resolve_optimizer(optimizer)(target, logfile=None)
        ...
        return StructureRelaxationResult(
            structure=_structure_mapping(atoms),
            energy=energy,
...
def relax_structure_alignn_ff(
    ...
    *,
    ...
    return ALIGNNFFStructureRelaxer(model_name=model_name).relax(
        structure, optimizer=optimizer, fmax=fmax, max_steps=max_steps, relax_cell=relax_cell
    )


__all__ = ["ALIGNNFFStructureRelaxer", "OptimizerName", "relax_structure_alignn_ff"]
