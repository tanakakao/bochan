"""Relax multiple structures with MACE and rank them with a bochan/BoTorch model."""
...
class MACERelaxationRanker:
    """Relax candidate structures with MACE, then rank them with bochan posterior scores.

    ``model_factory`` receives the relaxed structures in exactly the order used to
    construct the first input column (0..n-1). This is deliberate: relaxation
    changes atomic coordinates, so an existing model with a stale structure bank
    ...
    def run(
        ...
        *,
        ...
        """Relax every structure and rank the relaxed bank with one scalar posterior."""
