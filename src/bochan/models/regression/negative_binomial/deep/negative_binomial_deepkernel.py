...
def make_negative_binomial_feature_extractor(input_dim: int, output_dim: Optional[int] = None, ext_type: str = "DEFAULT") -> nn.Module:
    """Negative Binomial DeepKernel 用 feature extractor を作る。"""
    output_dim = input_dim if output_dim is None else int(output_dim)
    hidden_dims = [max(input_dim * 8, output_dim), max(input_dim * 4, output_dim), max(input_dim * 2, output_dim)]
...
class _DeepKernelNegativeBinomialSVGP(ApproximateGP):
    """Deep Kernel Negative Binomial 用 latent SVGP。"""
    ...
    def __init__(
        ...
        *,
        ...
        self.feature_extractor = feature_extractor or make_negative_binomial_feature_extractor(input_dim=input_dim, output_dim=input_dim, ext_type=ext_type)
...
class _DeepKernelMixedNegativeBinomialSVGP(ApproximateGP):
    """mixed Deep Kernel Negative Binomial 用 latent SVGP。"""
    ...
    def __init__(
        ...
        *,
        ...
        self.mean_module = mean_module or ConstantMean()
        self.covar_module = covar_module or build_mixed_negative_binomial_kernel(d=d, cat_dims=self.cat_dims, batch_shape=torch.Size())
        self.train_inputs = (train_X,)
        self.train_targets = train_Y
...
class DeepKernelNegativeBinomialGPModel(_BaseNegativeBinomialGPModel):
    """Deep Kernel Learning 版 Negative Binomial GP。"""
    ...
    def __init__(
        ...
        *,
        ...
        train_X_tf = apply_input_transform_for_training(train_X, input_transform, name="DeepKernelNegativeBinomialGPModel.input_transform")
        ...
        latent_model = _DeepKernelNegativeBinomialSVGP(
            train_X=train_X_tf,
            train_Y=train_Y,
            ext_type=ext_type,
...
class DeepKernelNegativeBinomialMixedGPModel(_BaseNegativeBinomialGPModel):
    """mixed Deep Kernel Learning 版 Negative Binomial GP。"""
    ...
    def __init__(
        ...
        *,
        ...
        train_Y = prepare_count_targets(train_Y, train_X)
        input_transform = to_device_dtype_transform(clone_input_transform(input_transform), train_X)
        train_X_tf = apply_input_transform_for_training(train_X, input_transform, cat_dims=cat_dims, name="DeepKernelNegativeBinomialMixedGPModel.input_transform")
        likelihood = likelihood or NegativeBinomialLogLikelihood(
            link=link,
            init_total_count=init_total_count,
            learn_total_count=learn_total_count,
        ...
        latent_model = _DeepKernelMixedNegativeBinomialSVGP(
            train_X=train_X_tf,
            train_Y=train_Y,
            cat_dims=cat_dims,
            ext_type=ext_type,
...
__all__ = [
    ...
    "make_negative_binomial_feature_extractor",
]
