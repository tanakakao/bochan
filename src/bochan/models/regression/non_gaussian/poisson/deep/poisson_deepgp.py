...
class _BasePoissonDeepGPModel(DeepGP, GPyTorchModel):
    """Poisson DeepGP wrapper の共通基底。"""
    ...
    def transform_inputs(self, X: Tensor) -> Tensor:
        """raw-space X を DeepGP 内部入力へ変換する。"""
...
class PoissonDeepGPModel(_BasePoissonDeepGPModel):
    ...
    def __init__(
        ...
        *,
        ...
        for h in list_hidden_dims[1:]:
            ...
            current_dim = int(h)
        self.extra_layers = ModuleList(extra_layers)
        ...
        self.likelihood = likelihood or PoissonLogLikelihood(link=link, exp_clip=exp_clip, min_rate=min_rate)
    ...
    def condition_on_observations(self, X: Tensor, Y: Tensor, **kwargs: Any) -> "PoissonDeepGPModel":
        ...
        new_model.eval(); new_model.likelihood.eval()
        return new_model
