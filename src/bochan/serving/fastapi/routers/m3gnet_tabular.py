"""FastAPI endpoints for structure-aware M3GNet tabular optimization."""
...
from ..services.m3gnet_tabular import (
...
router = APIRouter(prefix="/tabular/m3gnet/models", tags=["tabular", "m3gnet"])


def _validate_m3gnet_optimizer(optimizer: Any, *, model_id: str | None = None) -> Any:
    """Validate that one tabular optimizer carries the M3GNet structure contract."""
    ...
    if model_type not in _M3GNET_MODEL_TYPES:
        ...
        raise TypeError(f"{prefix}is not an M3GNet tabular model.")
    structure = getattr(optimizer, "structure", None)
    if structure is None or not bool(getattr(structure, "enabled", False)):
        raise TypeError("M3GNet tabular model is missing its fitted structure contract.")
    if getattr(structure, "graph_builder", None) is not None:
        raise TypeError("M3GNet tabular model must not contain an ALIGNN graph builder.")
    if getattr(optimizer, "dataset", None) is None or optimizer.bo.bundle is None:
        raise TypeError("M3GNet tabular model is not fitted.")
    return optimizer
...
def predict_m3gnet_tabular_model(
    model_id: str,
    ...
    optimizer = _get_optimizer(store, model_id)
    try:
        return m3gnet_predict_response(model_id, optimizer, request)
    except (KeyError, TypeError, ValueError) as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
def _candidate_endpoint(
    ...
    *,
    ...
    optimizer = _get_optimizer(store, model_id)
    try:
        return m3gnet_candidate_response(model_id, optimizer, request, use_ask=use_ask)
    except KeyError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
...
def generate_m3gnet_tabular_candidates(
    model_id: str,
    request: M3GNetTabularCandidateRequest,
    store: TabularOptimizerStore = TABULAR_STORE_DEP,
) -> TabularCandidateResponse:
...
def ask_m3gnet_tabular_candidates(
    model_id: str,
    request: M3GNetTabularCandidateRequest,
    store: TabularOptimizerStore = TABULAR_STORE_DEP,
) -> TabularCandidateResponse:
...
def save_m3gnet_tabular_model(
    model_id: str,
    request: SaveModelRequest,
    store: TabularOptimizerStore = TABULAR_STORE_DEP,
    file_store: FileOptimizerStore = FILE_STORE_DEP,
) -> SaveModelResponse:
    """Save an M3GNet tabular optimizer in the common trusted artifact format."""
    ...
    try:
        path = save_tabular_artifact(
            ...
            metadata={
                "surface": "fastapi-m3gnet",
                "source_model_id": model_id,
                "model_family": "m3gnet",
            },
        )
        m3gnet_metadata = build_m3gnet_fit_response(model_id, optimizer).metadata.get(
            "m3gnet", {}
        )
...
def load_m3gnet_tabular_model(
    request: LoadModelRequest,
    store: TabularOptimizerStore = TABULAR_STORE_DEP,
    file_store: FileOptimizerStore = FILE_STORE_DEP,
) -> TabularModelLoadResponse:
    """Load a trusted M3GNet tabular artifact and restore its structure contract."""
    ...
    try:
        ...
        _validate_m3gnet_optimizer(optimizer)
        model_id = store.add(optimizer)
        response = build_m3gnet_fit_response(model_id, optimizer).model_dump()
