"""FastAPI application service for tabular CHGNet models."""
...
_CHECKPOINT_ROOT_ENV = "BOCHAN_CHGNET_CHECKPOINT_ROOT"
...
def _model_config_from_request(request: CHGNetTabularFitModelRequest) -> dict[str, Any]:
    """Resolve API-safe CHGNet model config and allowlisted checkpoint IDs."""
...
def fit_chgnet_tabular_optimizer(
    request: CHGNetTabularFitModelRequest,
) -> TabularBayesianOptimizer:
    """Fit one CHGNet tabular optimizer from JSON data and crystal structures."""
...
def _chgnet_output_models(model: Any) -> list[Any]:
    """Return the single CHGNet model or independent ModelListGP submodels."""
...
def build_chgnet_fit_response(
    ...
    representative_model = model if correlated else _chgnet_output_models(model)[0]
    ...
    if correlated:
        ...
        output_models = _chgnet_output_models(model)
    ...
    metadata["chgnet"] = to_serializable(
        {
