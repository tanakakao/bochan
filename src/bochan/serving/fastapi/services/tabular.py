"""Tabular application service shared by thin FastAPI endpoints."""
...
def to_dataframe(data: Any) -> Any:
    """Convert a JSON tabular payload to a non-empty pandas DataFrame."""
...
def _experiment_failure_config(
    request: TabularFitModelRequest,
) -> ExperimentFailureConfig | None:
    """Convert JSON-safe experiment-failure settings to the core config."""
...
def build_fit_response(
    ...
    """Serialize one fitted tabular optimizer."""
...
def compute_feature_importance_response(
    model_id: str,
    optimizer: TabularBayesianOptimizer,
    request: TabularFeatureImportanceRequest,
) -> TabularFeatureImportanceResponse:
    """Compute feature importance and serialize optional visualization views."""

    frame = to_dataframe(request.data) if request.data is not None else None
...
def predict_response(
    ...
    """Run one tabular prediction and serialize its result."""
...
def candidate_response(
    ...
    *,
    ...
    """Generate tabular candidates and serialize the candidate DataFrame."""
