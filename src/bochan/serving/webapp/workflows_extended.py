"""Extended Web workflow with feature constraints and selectable search backends."""
...
def run_regression_web_workflow(request: Any, store: Any) -> dict[str, Any]:
    """Fit target-specific models and generate constrained candidates."""
    ...
    if acquisition_family == "bayesian_optimization":
        if not multi_objective:
            if acq_key not in {"ei", "nei", "pi", "ucb"}:
                raise ValueError(
                    "Single-objective Bayesian optimization requires EI, PI, or "
                    f"UCB, got {acq_name}."
                )
        ...
        if acq_key not in {"variance", "predictiveentropy", "bald", "nipv", "qnipv"}:
            raise ValueError(
                "Active learning supports variance, predictive_entropy, BALD, or "
                f"NIPV, got {acq_name}."
            )
        ...
        if acq_key not in {"straddle", "boundaryvariance", "icu"}:
            raise ValueError(
                "Level-set estimation supports straddle, boundary_variance, or "
                f"ICU, got {acq_name}."
            )
        objective_config = None
        acqf_kwargs.setdefault(
    ...
    for row in rows:
        feature_results = feature_constraint_results(row["values"], feature_constraints)
