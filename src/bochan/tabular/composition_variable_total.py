"""Variable-total composition transformations for tabular optimization."""
...
class CompositionVariableTotalTransform:
    """Own variable-total composition normalization and model-space transforms."""
    ...
    def normalize_sites(
        ...
        *,
        ...
        """Normalize ``total_bounds`` settings around the base composition-site schema."""
    ...
    def make_site_search_space(
        ...
        *,
        ...
        """Return no fixed-total search space for variable-total sites."""
    ...
    def formula_site_totals(
        ...
        """Extract absolute totals from formula-column inputs."""
    ...
    def site_totals_from_frame(
        ...
        """Resolve one variable site's total feature from model input data."""
    ...
    def collect_site_totals(self, owner: Any, data: Any) -> dict[str, np.ndarray]:
        """Collect available total features before the base composition transform."""
    ...
    @staticmethod
    def inject_site_totals(
        transformed: Any,
        totals: Mapping[str, np.ndarray],
        composition_sites: Mapping[str, Mapping[str, Any]],
    ) -> Any:
        """Inject collected absolute totals into transformed model features."""

        for site_name, values in totals.items():
            feature = composition_sites[site_name]["total_feature"]
            transformed.loc[:, feature] = values
        return transformed

    @staticmethod
    def replace_input_cols(owner: Any, input_cols: Sequence[Any] | None) -> list[Any] | None:
        """Replace raw composition columns with representation and total features."""
    ...
    def complete_bounds(owner: Any, expanded: Any) -> Any:
        """Add variable-total and fraction-coordinate bounds to transformed bounds."""
    ...
    def dynamic_search_space(
        ...
        """Build a repair search space for one candidate-specific site total."""
    ...
    def restore(
        ...
        *,
        ...
        """Restore variable-total sites from transformed candidate coordinates."""
