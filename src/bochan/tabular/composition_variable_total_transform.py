"""Variable-total composition transforms for the canonical tabular optimizer."""
...
class CompositionVariableTotalTransform:
    """Apply variable composition totals without owning optimizer state."""
    ...
    def normalize_sites(
        ...
        *,
        ...
        for raw_name, raw_config in sites.items():
            ...
            if len(pair) != 2:
                raise ValueError(f"total_bounds for site {name!r} must contain two values.")
        ...
        for name, config in normalized.items():
            ...
            if all(element in config["bounds"] for element in config["elements"]):
                upper_sum = sum(float(config["bounds"][element][1]) for element in config["elements"])
                if upper_sum < total_bounds[1] - 1e-12:
                    raise ValueError(
                        f"Upper component bounds at site {name!r} allow a total of at most "
                        f"{upper_sum}, so the upper end of total_bounds={total_bounds!r} is infeasible."
                    )
        total_features = [
            config["total_feature"] for config in normalized.values() if config["variable_total"]
        ]
    ...
    def formula_site_totals(formulas: Any, config: Mapping[str, Any]) -> np.ndarray:
        ...
        weight_basis = str(config["normalization"]).lower() in {
            "weight_fraction", "weight", "mass_fraction"
        }
        for formula in formulas:
            parsed = parse_formula(str(formula))
            if weight_basis:
                total = sum(float(amount) * ATOMIC_WEIGHTS[element] for element, amount in parsed.items())
            else:
    ...
    def site_totals_from_frame(
        ...
        *,
        ...
        if total_feature in data.columns:
            ...
            source_columns = site_source_columns(config)
            if not all(column in data.columns for column in source_columns):
                return None
            values = numeric_site_values(data, site_name, config).sum(axis=1)
        else:
            column = config["column"]
            if column not in data.columns:
                return None
            values = cls.formula_site_totals(data.loc[:, column], config)
        if not np.isfinite(values).all() or np.any(values <= 0.0):
            raise ValueError(f"Composition totals for site {site_name!r} must be finite and positive.")
        return values
        ...
        *,
        ...
        for site_name, config in composition_sites.items():
            if not config.get("variable_total"):
                continue
            values = cls.site_totals_from_frame(
                data, site_name, config,
                site_source_columns=site_source_columns,
                numeric_site_values=numeric_site_values,
            )
    ...
    def replace_input_cols(
        ...
        *,
        ...
        source_to_site = {
            ...
            for site_name, config in composition_sites.items()
            for column in site_source_columns(config)
        }
    ...
    @staticmethod
    def dynamic_search_space(config: Mapping[str, Any], total: float) -> CompositionSearchSpace:
        return CompositionSearchSpace(
            components=config["elements"], total=float(total), bounds=config["bounds"],
            steps=config["steps"], min_active_components=config["min_components"],
    ...
    def inverse_compositions(
        ...
        *,
        composition_sites: Mapping[str, Mapping[str, Any]],
        composition_transformers: Mapping[str, Any],
        multi_site_composition_enabled: bool,
        repair: bool,
    ) -> Any:
        import pandas as pd
        ...
        for site_name, config in composition_sites.items():
            ...
            if total_feature not in candidates:
                raise KeyError(f"Missing total feature {total_feature!r} for site {site_name!r}.")
            lower, upper = config["total_bounds"]
            totals = np.clip(candidates.loc[:, total_feature].to_numpy(dtype=float), lower, upper)
            transformer = composition_transformers[site_name]
            elements = transformer._require_fitted()
            representation_names = transformer._representation_names(elements)
            missing = [name for name in representation_names if name not in candidates.columns]
            if missing:
                raise KeyError(f"Missing model-space columns for site {site_name!r}: {missing!r}.")
            ...
            for basis_row, total in zip(basis_fractions, totals, strict=True):
                ...
                if repair:
                    composition = cls.dynamic_search_space(config, float(total)).repair(composition)
                rows.append(composition)
            absolute = np.asarray([[row[element] for element in elements] for row in rows], dtype=float)
            ...
            if str(config["normalization"]).lower() in {"weight_fraction", "weight", "mass_fraction"}:
            ...
            restored.loc[:, config["column"]] = [
                format_formula(dict(zip(elements, row, strict=True)), order=elements, precision=config["precision"])
...
__all__ = ["CompositionVariableTotalTransform"]
