...
def _has_derived_m3gnet_model_cls(model_cls: Any) -> bool:
    if model_cls is None:
        return False
    return (
        getattr(model_cls, "__name__", None) in _DERIVED_M3GNET_MODEL_CLASSES
...
def _validate_model_config(
    ...
    *,
    ...
    if not derived_config and config.input_type not in (None, expected_input_type):
        raise ValueError(
            "Tabular M3GNet with the configured process columns requires "
    ...
    if not derived_config and configured_cat_dims:
        raise ValueError(
            "Tabular M3GNet derives cat_dims from categorical process columns; "
            "do not pass cat_dims explicitly."
        )
    if config.input_transform is not None:
        raise ValueError(
            "Tabular M3GNet derives process-only normalization automatically; "
    ...
    if transform_config is not None:
        if bool(transform_config.perturbation):
            raise ValueError("Tabular M3GNet does not support input perturbation.")
        if not bool(transform_config.normalize):
            raise ValueError("Tabular M3GNet currently requires process normalization.")
        if transform_config.bounds is not None:
            raise ValueError(
                "Tabular M3GNet derives process normalization from tabular bounds; "
    ...
    if existing_structures is not None and existing_structures is not structures:
        raise ValueError(
            "Tabular M3GNet derives structures from structure_catalog; "
...
def _configure_single_model(
    ...
    *,
    ...
    if dkl:
        if is_mixed:
            from bochan.models.regression.gaussian.deep import M3GNetMixedDKLModel

            model_cls = M3GNetMixedDKLModel
        else:
            from bochan.models.regression.gaussian.deep import M3GNetDKLModel

            model_cls = M3GNetDKLModel
    elif is_mixed:
        from bochan.models.regression.gaussian.deep import M3GNetMixedGPModel

        model_cls = M3GNetMixedGPModel
    else:
        from bochan.models.regression.gaussian.deep import M3GNetGPModel

        model_cls = M3GNetGPModel
...
def configure_tabular_m3gnet(owner: Any) -> bool:
    ...
    if owner.composition.enabled:
        raise ValueError(
            "Tabular M3GNet accepts crystal structure plus process variables; "
            "composition_sites cannot be combined with M3GNet yet."
        )
    ...
    if owner.structure.graph_builder is not None:
        raise ValueError(
            "structure_graph_builder is ALIGNN-specific and must be omitted for M3GNet."
        )

    source = owner.source_data_config
    if source.input_cols is None:
        raise ValueError(
            "Tabular M3GNet requires explicit input_cols so structure_col can be placed "
    ...
    if source.bounds is not None and not isinstance(source.bounds, Mapping):
        raise ValueError(
            "Tabular M3GNet requires column-addressed bounds when bounds are supplied."
        )
    ...
    if missing_bounds:
        raise ValueError(
            "Tabular M3GNet requires bounds for every continuous process variable; "
