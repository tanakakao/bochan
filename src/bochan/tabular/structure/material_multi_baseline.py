...
_DERIVED_CONFIG_MARKER = "_material_multiple_baseline_derived"
...
def _is_derived_multi_output_config(config: Any) -> bool:
    """Return whether a multi-output config was generated by this router."""

    multi_output_config = getattr(config, "multi_output_config", None)
    if multi_output_config is None:
        return False
    wrapper_kwargs = getattr(multi_output_config, "wrapper_kwargs", None)
    return (
        getattr(multi_output_config, "wrapper_factory", None) is _build_multiple_baseline_wrapper
        and isinstance(wrapper_kwargs, Mapping)
        and wrapper_kwargs.get(_DERIVED_CONFIG_MARKER) is True
    )
...
def configure_tabular_multiple_baseline(owner: Any) -> bool:
    ...
    if config.multi_output_config is not None:
        if _is_derived_multi_output_config(config):
            return True
        raise ValueError(
