...
"""Binary classification deep multi-output smoke tests.

The multi-output model is built as a list of independent deep single-output
binary classifiers and wrapped by ``MultiOutputBinaryClassificationModel``.
This mirrors ``test_binary_classification_base_multi_output.py`` while swapping
only the single-output model class and fit helper.

Covered deep families:
    - DeepGP
    - DeepKernel

Covered acquisition families are imported from the base multi-output test:
    - active learning
...
DeepKind = Literal["deepgp", "deepkernel"]


def _fit_deep_single_output_model(
    model: Any,
    *,
    deep_kind: DeepKind,
    num_epochs: int,
    ...
    """DeepGP / DeepKernel の single-output classifier を軽量 fit する。"""
    if deep_kind == "deepgp":
        fit_deepgp_mll(
            model.make_mll(),
    ...
    raise ValueError(f"Unknown deep_kind: {deep_kind}")
...
def _assert_deep_single_submodel_training(
    submodel: Any,
    ...
    *,
    ...
    """MultiOutput wrapper 内の deep single-output submodel を確認する。"""
    submodel.eval()
    ...
    # DeepKernel は inner SVGP が transformed-space を保持する。
    # DeepGP wrapper は raw-space train_inputs を wrapper 側で保持するため、
    # inner の詳細まではここでは固定しない。
    if deep_kind == "deepkernel":
...
def _assert_deep_multi_output_model_training(
    ...
    *,
    ...
    """Deep single-output model list から作った multi-output wrapper を確認する。"""
    model.eval()
    n, m = train_y.shape
    ...
    for j, submodel in enumerate(model.models):
        _assert_deep_single_submodel_training(
            submodel=submodel,
            train_x=train_x,
            train_y_j=train_y[:, [j]],
            cat_dims=cat_dims,
...
def create_deep_multi_output_binary_model_bundle(
    *,
    deep_kind: DeepKind,
    cat: bool = False,
    n: int = 16,
    ...
    """出力ごとに deep single-output classifier を fit して multi-output wrapper を作る。"""
    ...
    for j in range(train_y.shape[-1]):
        ...
        if deep_kind == "deepgp":
            ...
            kwargs.update({"num_inducing_points": 8})
            if cat:
                kwargs["cat_dims"] = cat_dims

        torch.manual_seed(j)
        submodel = model_cls(**kwargs)
        _fit_deep_single_output_model(
            submodel,
            deep_kind=deep_kind,
            num_epochs=num_epochs,
            lr=0.01,
    ...
    _assert_deep_multi_output_model_training(
        model=model,
        train_x=train_x,
        train_y=train_y,
        cat_dims=cat_dims,
...
@pytest.fixture(scope="module", params=["deepgp", "deepkernel"])
def deep_multi_output_binary_model_bundle(request: pytest.FixtureRequest) -> dict[str, Any]:
    """pytest 用: 通常 deep multi-output binary classification model。"""
    return create_deep_multi_output_binary_model_bundle(
        deep_kind=request.param,
        cat=False,
        n=16,
        d=5,
        m=N_OUTPUTS,
...
@pytest.fixture(scope="module", params=["deepgp", "deepkernel"])
def deep_multi_output_binary_mixed_model_bundle(request: pytest.FixtureRequest) -> dict[str, Any]:
    """pytest 用: mixed deep multi-output binary classification model。"""
    return create_deep_multi_output_binary_model_bundle(
        deep_kind=request.param,
        cat=True,
        n=16,
...
def test_deep_multi_output_binary_model_basic_behavior(
    deep_multi_output_binary_model_bundle: dict[str, Any],
) -> None:
    _assert_deep_multi_output_model_training(
        model=deep_multi_output_binary_model_bundle["model"],
        train_x=deep_multi_output_binary_model_bundle["train_x"],
        train_y=deep_multi_output_binary_model_bundle["train_y"],
        cat_dims=deep_multi_output_binary_model_bundle["cat_dims"],
        deep_kind=deep_multi_output_binary_model_bundle["deep_kind"],
    )


def test_deep_multi_output_binary_mixed_model_basic_behavior(
    deep_multi_output_binary_mixed_model_bundle: dict[str, Any],
) -> None:
    _assert_deep_multi_output_model_training(
        model=deep_multi_output_binary_mixed_model_bundle["model"],
        train_x=deep_multi_output_binary_mixed_model_bundle["train_x"],
        train_y=deep_multi_output_binary_mixed_model_bundle["train_y"],
        cat_dims=deep_multi_output_binary_mixed_model_bundle["cat_dims"],
        deep_kind=deep_multi_output_binary_mixed_model_bundle["deep_kind"],
    )


def test_deep_multi_output_binary_acquisition_forward_shapes(
    deep_multi_output_binary_model_bundle: dict[str, Any],
) -> None:
    model = deep_multi_output_binary_model_bundle["model"]
    train_x = deep_multi_output_binary_model_bundle["train_x"]
    X = make_random_batch(
        deep_multi_output_binary_model_bundle["bounds"],
        batch_size=4,
        q=2,
    )
...
def test_deep_multi_output_binary_mixed_acquisition_forward_shapes(
    deep_multi_output_binary_mixed_model_bundle: dict[str, Any],
) -> None:
    model = deep_multi_output_binary_mixed_model_bundle["model"]
    train_x = deep_multi_output_binary_mixed_model_bundle["train_x"]
    X = make_random_mixed_batch(
        deep_multi_output_binary_mixed_model_bundle["bounds"],
        deep_multi_output_binary_mixed_model_bundle["cat_dims"],
        batch_size=4,
        q=2,
    )
...
def test_deep_multi_output_binary_family_case_coverage(
    deep_multi_output_binary_model_bundle: dict[str, Any],
) -> None:
    model = deep_multi_output_binary_model_bundle["model"]
    train_x = deep_multi_output_binary_model_bundle["train_x"]
...
def test_deep_multi_output_binary_constraint_scenario_coverage(
    deep_multi_output_binary_model_bundle: dict[str, Any],
) -> None:
    model = deep_multi_output_binary_model_bundle["model"]
    train_x = deep_multi_output_binary_model_bundle["train_x"]
    bounds = deep_multi_output_binary_model_bundle["bounds"]
...
@pytest.mark.slow
def test_deep_multi_output_binary_optimize_acqf_representative_smoke(
    deep_multi_output_binary_model_bundle: dict[str, Any],
) -> None:
    model = deep_multi_output_binary_model_bundle["model"]
    train_x = deep_multi_output_binary_model_bundle["train_x"]
...
@pytest.mark.slow
def test_deep_multi_output_binary_mixed_optimize_acqf_mixed_representative_smoke(
    deep_multi_output_binary_mixed_model_bundle: dict[str, Any],
) -> None:
    model = deep_multi_output_binary_mixed_model_bundle["model"]
    train_x = deep_multi_output_binary_mixed_model_bundle["train_x"]
    bounds = deep_multi_output_binary_mixed_model_bundle["bounds"]
    cat_id = deep_multi_output_binary_mixed_model_bundle["cat_dims"][0]
...
@pytest.mark.slow
def test_deep_multi_output_binary_optimizer_constraint_case_smoke(
    deep_multi_output_binary_model_bundle: dict[str, Any],
) -> None:
    model = deep_multi_output_binary_model_bundle["model"]
    train_x = deep_multi_output_binary_model_bundle["train_x"]
...
@pytest.mark.slow
def test_deep_multi_output_binary_mixed_optimizer_constraint_case_smoke(
    deep_multi_output_binary_mixed_model_bundle: dict[str, Any],
) -> None:
    model = deep_multi_output_binary_mixed_model_bundle["model"]
    train_x = deep_multi_output_binary_mixed_model_bundle["train_x"]
...
def run_jupyter_forward_check(
    *,
    ...
    bundle = create_deep_multi_output_binary_model_bundle(
        deep_kind=deep_kind,
        cat=cat,
        n=n,
        d=d,
    ...
    print("=" * 80)
    print(f"Jupyter deep multi-output binary forward check deep_kind={deep_kind}, cat={cat}")
...
def run_jupyter_all_forward_checks(*, num_epochs: int = 4, verbose_forward_detail: bool = False) -> None:
    for deep_kind in ["deepgp", "deepkernel"]:
    ...
    print("all deep multi-output binary forward checks passed.")
