...
def create_binary_deepgp_model_bundle(*, cat: bool = False) -> dict[str, Any]:
    ...
    fit_deepgp_mll(mll, num_epochs=20)
...
def test_binary_deepgp_mixed_acquisition_forward_shapes(binary_deepgp_mixed_model_bundle: dict[str, Any]) -> None:
    ...
    X = make_random_mixed_batch(binary_deepgp_mixed_model_bundle["bounds"], binary_deepgp_mixed_model_bundle["cat_dims"], batch_size=4, q=2)
...
@pytest.mark.slow
def test_binary_deepgp_acquisition_optimize_acqf_smoke(binary_deepgp_model_bundle: dict[str, Any]) -> None:
    model = binary_deepgp_model_bundle["model"]
    train_x = binary_deepgp_model_bundle["train_x"]
...
@pytest.mark.slow
def test_binary_deepgp_mixed_acquisition_optimize_acqf_smoke(binary_deepgp_mixed_model_bundle: dict[str, Any]) -> None:
    model = binary_deepgp_mixed_model_bundle["model"]
    train_x = binary_deepgp_mixed_model_bundle["train_x"]
...
@pytest.mark.slow
def test_binary_deepgp_optimizer_constraint_compatibility_smoke() -> None:
    bundle = create_binary_deepgp_model_bundle(cat=False)
...
@pytest.mark.slow
def test_binary_deepgp_mixed_optimizer_constraint_compatibility_smoke() -> None:
    bundle = create_binary_deepgp_model_bundle(cat=True)
    model, train_x, bounds = bundle["model"], bundle["train_x"], bundle["bounds"]
    q, d = 2, train_x.shape[-1]
