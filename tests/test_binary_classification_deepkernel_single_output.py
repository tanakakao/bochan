...
"""Binary classification DeepKernel single-output smoke tests.

This module reuses the toy data, acquisition cases, optimizer wrappers, and
constraint assertions from ``test_binary_classification_base_single_output.py``.
The DeepKernel-specific parts are limited to model construction, the fit helper,
and assertions for the wrapper / inner SVGP train-data contract.
"""
...
def _build_input_transform(
    ...
    """DeepKernel / mixed DeepKernel 共通の input_transform を作る。"""
...
def _fit_deepkernel_binary_model(
    model: Any,
    *,
    num_epochs: int,
    lr: float = 0.01,
) -> None:
    """DeepKernel binary classification 専用 fit helper を軽量設定で実行する。"""
    fit_deepkernel_mll(
        model.make_mll(),
...
def _assert_deepkernel_model_training(
    ...
    *,
    ...
    # DeepKernel wrapper は raw-space train_inputs を保持し、
    # inner SVGP 側には transformed train_inputs を保持する。
    ...
    if cat_dims:
        ...
        assert hasattr(model.model, "cat_dims")
...
def create_binary_deepkernel_model_bundle(
    *,
    cat: bool = False,
    n: int = 16,
    d: int = 5,
    ...
    """Jupyter / pytest 共通で使う binary DeepKernel model 作成関数。"""
    ...
    torch.manual_seed(0)
    if cat:
        model = DeepKernelBinaryClassificationMixedGPModel(
            train_X=train_x,
            train_Y=train_y,
...
def binary_deepkernel_model_bundle() -> dict[str, Any]:
    """pytest 用: 通常 binary DeepKernel model。"""
    return create_binary_deepkernel_model_bundle(
        cat=False,
        n=16,
        d=5,
        num_epochs=8,
...
def binary_deepkernel_mixed_model_bundle() -> dict[str, Any]:
    """pytest 用: mixed binary DeepKernel model。"""
    return create_binary_deepkernel_model_bundle(
        cat=True,
        n=16,
        d=5,
...
def _representative_acquisition_cases(
    ...
    """DeepKernel optimizer smoke test 用に代表 acquisition だけを返す。"""
    names = {
        "predictive_entropy",
        "latent_straddle",
        "pof",
...
def _representative_constraint_cases(bounds: torch.Tensor) -> list[dict[str, Any]]:
    """DeepKernel optimizer constraint smoke test 用の軽量ケースだけを返す。"""
...
def _optimizer_constraint_scenarios(
    ...
    *,
    ...
    prefix = "deepkernel_mixed" if mixed else "deepkernel"
    return [
        (
            acq_cls,
            kwargs,
...
def test_binary_deepkernel_model_basic_behavior(
    ...
    _assert_deepkernel_model_training(
        ...
        train_y=binary_deepkernel_model_bundle["train_y"],
        cat_dims=binary_deepkernel_model_bundle["cat_dims"],
    )


def test_binary_deepkernel_mixed_model_basic_behavior(
    ...
    _assert_deepkernel_model_training(
        ...
        train_y=binary_deepkernel_mixed_model_bundle["train_y"],
        cat_dims=binary_deepkernel_mixed_model_bundle["cat_dims"],
    )


def test_binary_deepkernel_acquisition_forward_shapes(
    binary_deepkernel_model_bundle: dict[str, Any],
) -> None:
    model = binary_deepkernel_model_bundle["model"]
    train_x = binary_deepkernel_model_bundle["train_x"]
    X = make_random_batch(
        binary_deepkernel_model_bundle["bounds"],
        batch_size=4,
        q=2,
    )
...
def test_binary_deepkernel_mixed_acquisition_forward_shapes(
    binary_deepkernel_mixed_model_bundle: dict[str, Any],
) -> None:
    model = binary_deepkernel_mixed_model_bundle["model"]
    train_x = binary_deepkernel_mixed_model_bundle["train_x"]
...
@pytest.mark.slow
def test_binary_deepkernel_optimizer_constraint_case_smoke(
    binary_deepkernel_model_bundle: dict[str, Any],
) -> None:
    model = binary_deepkernel_model_bundle["model"]
    train_x = binary_deepkernel_model_bundle["train_x"]
...
@pytest.mark.slow
def test_binary_deepkernel_mixed_optimizer_constraint_case_smoke(
    binary_deepkernel_mixed_model_bundle: dict[str, Any],
) -> None:
    model = binary_deepkernel_mixed_model_bundle["model"]
    train_x = binary_deepkernel_mixed_model_bundle["train_x"]
...
def run_jupyter_forward_check(
    *,
    ...
    print("=" * 80)
    print(f"Jupyter DeepKernel forward check cat={cat}")
...
def run_jupyter_optimize_acqf_all_acquisitions_check(
    *,
    ...
    print("=" * 100)
    print("Jupyter DeepKernel optimize_acqf check: all acquisitions")
...
def run_jupyter_optimize_acqf_mixed_all_acquisitions_check(
    *,
    ...
    print("=" * 100)
    print("Jupyter DeepKernel optimize_acqf_mixed check: all acquisitions")
    print(
        f"n={n}, d={d}, q={q}, "
...
def run_jupyter_all_checks(
    *,
    ...
    print("all DeepKernel Jupyter checks passed.")
