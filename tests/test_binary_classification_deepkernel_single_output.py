...
def test_binary_deepkernel_optimizer_constraint_case_smoke(
    ...
    """DeepKernel でも base 側と同じ constraint_case helper を使えることを確認する。"""
    model = binary_deepkernel_model_bundle["model"]
    train_x = binary_deepkernel_model_bundle["train_x"]
    bounds = binary_deepkernel_model_bundle["bounds"]
    ...
    for constraint_case in _representative_constraint_cases(bounds):
        case_id = f"deepkernel__binary_ucb__torch_adam__{constraint_case['case_id']}"
        with maybe_suppress_botorch_initial_warnings():
...
def test_binary_deepkernel_mixed_optimizer_constraint_case_smoke(
    ...
    """mixed DeepKernel でも constraint_case と fixed_features_list を併用できることを確認する。"""
    model = binary_deepkernel_mixed_model_bundle["model"]
    train_x = binary_deepkernel_mixed_model_bundle["train_x"]
    bounds = binary_deepkernel_mixed_model_bundle["bounds"]
    cat_id = binary_deepkernel_mixed_model_bundle["cat_dims"][0]
    ...
    for constraint_case in _representative_constraint_cases(bounds):
        case_id = f"deepkernel_mixed__binary_ucb__torch_adam__{constraint_case['case_id']}"
        with maybe_suppress_botorch_initial_warnings():
