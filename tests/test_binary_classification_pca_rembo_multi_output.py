...
"""Binary classification PCA / REMBO multi-output smoke tests.

Each output is modeled by an independent projected single-output binary
classifier and wrapped by ``MultiOutputBinaryClassificationModel``.  The
Jupyter runners support ``kind='pca'``, ``kind='rembo'``, or ``kind='all'``.
"""
...
from tests.test_binary_classification_pca_rembo_single_output import (
    N_COMPONENTS,
...
ProjectionKindOrAll = Literal["pca", "rembo", "all"]


def create_projected_multi_output_binary_model_bundle(
    *,
    kind: ProjectionKind,
    cat: bool = False,
    n: int = 18,
    ...
    for j in range(train_y.shape[-1]):
        ...
        _fit_projected_binary_model(submodel, num_epochs=num_epochs, lr=0.01)
    ...
    assert_multi_output_wrapper_training(
        ...
        train_y=train_y,
        cat_dims=cat_dims,
        submodel_assert_fn=_assert_projected_model_training,
        submodel_assert_extra_kwargs={"n_components": n_components},
    )
    return {
        "model": model,
        "train_x": train_x,
...
def projected_multi_output_binary_model_bundle(request: pytest.FixtureRequest) -> dict[str, Any]:
    return create_projected_multi_output_binary_model_bundle(kind=request.param, cat=False)


@pytest.fixture(scope="module", params=["pca", "rembo"])
def projected_multi_output_binary_mixed_model_bundle(request: pytest.FixtureRequest) -> dict[str, Any]:
    return create_projected_multi_output_binary_model_bundle(kind=request.param, cat=True)


def test_projected_multi_output_binary_model_basic_behavior(projected_multi_output_binary_model_bundle: dict[str, Any]) -> None:
    assert_multi_output_wrapper_training(
        model=projected_multi_output_binary_model_bundle["model"],
        train_x=projected_multi_output_binary_model_bundle["train_x"],
        train_y=projected_multi_output_binary_model_bundle["train_y"],
        cat_dims=projected_multi_output_binary_model_bundle["cat_dims"],
        submodel_assert_fn=_assert_projected_model_training,
        submodel_assert_extra_kwargs={"n_components": projected_multi_output_binary_model_bundle["n_components"]},
    )


def test_projected_multi_output_binary_mixed_model_basic_behavior(projected_multi_output_binary_mixed_model_bundle: dict[str, Any]) -> None:
    assert_multi_output_wrapper_training(
        model=projected_multi_output_binary_mixed_model_bundle["model"],
        train_x=projected_multi_output_binary_mixed_model_bundle["train_x"],
        train_y=projected_multi_output_binary_mixed_model_bundle["train_y"],
        cat_dims=projected_multi_output_binary_mixed_model_bundle["cat_dims"],
        submodel_assert_fn=_assert_projected_model_training,
        submodel_assert_extra_kwargs={"n_components": projected_multi_output_binary_mixed_model_bundle["n_components"]},
    )


def test_projected_multi_output_binary_acquisition_forward_shapes(projected_multi_output_binary_model_bundle: dict[str, Any]) -> None:
    model = projected_multi_output_binary_model_bundle["model"]
    train_x = projected_multi_output_binary_model_bundle["train_x"]
    X = make_random_batch(projected_multi_output_binary_model_bundle["bounds"], batch_size=4, q=2)
...
def test_projected_multi_output_binary_mixed_acquisition_forward_shapes(projected_multi_output_binary_mixed_model_bundle: dict[str, Any]) -> None:
    model = projected_multi_output_binary_mixed_model_bundle["model"]
    train_x = projected_multi_output_binary_mixed_model_bundle["train_x"]
    X = make_random_mixed_batch(
        projected_multi_output_binary_mixed_model_bundle["bounds"],
        projected_multi_output_binary_mixed_model_bundle["cat_dims"],
        batch_size=4,
        q=2,
    )
    for acq_cls, kwargs, case_id in multi_output_acquisition_cases(model, train_x):
...
@pytest.mark.slow
def test_projected_multi_output_binary_optimize_acqf_representative_smoke(projected_multi_output_binary_model_bundle: dict[str, Any]) -> None:
    model = projected_multi_output_binary_model_bundle["model"]
    train_x = projected_multi_output_binary_model_bundle["train_x"]
    bounds = projected_multi_output_binary_model_bundle["bounds"]
...
@pytest.mark.slow
def test_projected_multi_output_binary_mixed_optimize_acqf_mixed_representative_smoke(projected_multi_output_binary_mixed_model_bundle: dict[str, Any]) -> None:
    model = projected_multi_output_binary_mixed_model_bundle["model"]
    train_x = projected_multi_output_binary_mixed_model_bundle["train_x"]
    bounds = projected_multi_output_binary_mixed_model_bundle["bounds"]
    cat_id = projected_multi_output_binary_mixed_model_bundle["cat_dims"][0]
...
@pytest.mark.slow
def test_projected_multi_output_binary_optimizer_constraint_case_smoke(projected_multi_output_binary_model_bundle: dict[str, Any]) -> None:
    model = projected_multi_output_binary_model_bundle["model"]
    train_x = projected_multi_output_binary_model_bundle["train_x"]
    bounds = projected_multi_output_binary_model_bundle["bounds"]
...
@pytest.mark.slow
def test_projected_multi_output_binary_mixed_optimizer_constraint_case_smoke(projected_multi_output_binary_mixed_model_bundle: dict[str, Any]) -> None:
    model = projected_multi_output_binary_mixed_model_bundle["model"]
    train_x = projected_multi_output_binary_mixed_model_bundle["train_x"]
    bounds = projected_multi_output_binary_mixed_model_bundle["bounds"]
    cat_id = projected_multi_output_binary_mixed_model_bundle["cat_dims"][0]
...
def _iter_kinds(kind: ProjectionKindOrAll) -> list[ProjectionKind]:
    if kind == "all":
        return ["pca", "rembo"]
    if kind in {"pca", "rembo"}:
        return [kind]
    raise ValueError(f"Unknown projection kind: {kind}")
...
def run_jupyter_forward_check(
    *,
    ...
    bundle = create_projected_multi_output_binary_model_bundle(kind=kind, cat=cat, n=n, d=d, m=m, n_components=n_components, num_epochs=num_epochs)
    model = bundle["model"]
    ...
    print("=" * 80)
    print(f"Jupyter {kind.upper()} multi-output binary forward check cat={cat}")
...
def run_jupyter_all_forward_checks(
    *,
    kind: ProjectionKindOrAll = "all",
    num_epochs: int = 4,
    verbose_forward_detail: bool = False,
) -> None:
    for current_kind in _iter_kinds(kind):
        run_jupyter_forward_check(kind=current_kind, cat=False, num_epochs=num_epochs, verbose_forward_detail=verbose_forward_detail)
        run_jupyter_forward_check(kind=current_kind, cat=True, num_epochs=num_epochs, verbose_forward_detail=verbose_forward_detail)
    print(f"all {kind} projected multi-output binary forward checks passed.")
...
def run_jupyter_optimize_all_acquisitions_check(
    *,
    ...
    print("=" * 100)
    print(f"Jupyter {kind.upper()} {prefix}multi-output optimize check: all acquisitions")
    print(f"n={n}, d={d}, m={m}, n_components={n_components}, q={q}, num_epochs={num_epochs}, num_acquisitions={len(cases)}")
    print("=" * 100)
    ...
    for acq_cls, kwargs, case_id in cases:
        display_id = f"{kind}_{prefix}optimize_all__{case_id}"
        try:
...
def run_jupyter_optimizer_constraint_support_check(
    *,
    ...
    print("=" * 100)
    print(f"Jupyter {kind.upper()} {prefix}multi-output optimizer / constraint support check")
    print(f"n={n}, d={d}, m={m}, n_components={n_components}, q={q}, num_epochs={num_epochs}, full_matrix={full_matrix}, num_cases={len(scenarios)}")
    print("=" * 100)

    for acq_cls, kwargs, _, optimize_func, optimize_method, constraint_case, case_id in scenarios:
        display_case_id = f"{kind}_{case_id}"
        try:
            with maybe_suppress_botorch_initial_warnings(suppress=suppress_botorch_warnings):
                if cat:
            ...
            assert_optimizer_support_result(cands=cands, acq_value=acq_value, bounds=bounds, q=q, d=train_x.shape[-1], constraint_case=constraint_case, case_id=display_case_id)
            if cat:
                assert cat_id is not None and cat_values is not None
                assert torch.isin(cands[:, cat_id], cat_values).all(), display_case_id
            print(f"[OK] {display_case_id} cands.shape={tuple(cands.shape)} acq_value={acq_value}" if verbose_ok_detail else f"[OK] {display_case_id}")
            ...
            print(f"[NG] {display_case_id} {type(exc).__name__}")
            print(str(exc))
            failed_cases.append((display_case_id, exc))
            if not continue_on_error:
                raise
...
def run_jupyter_all_checks(
    *,
    ...
    """PCA / REMBO multi-output binary classification の Jupyter 一括確認 helper。"""
    for current_kind in _iter_kinds(kind):
        run_jupyter_forward_check(kind=current_kind, cat=False, n=n, d=d, m=m, n_components=n_components, num_epochs=num_epochs, q=q, verbose_forward_detail=verbose_forward_detail)
        run_jupyter_forward_check(kind=current_kind, cat=True, n=n, d=d, m=m, n_components=n_components, num_epochs=num_epochs, q=q, verbose_forward_detail=verbose_forward_detail)

        if run_optimize:
            run_jupyter_optimize_all_acquisitions_check(kind=current_kind, cat=False, n=n, d=d, m=m, n_components=n_components, num_epochs=num_epochs, q=q, continue_on_error=continue_on_error, suppress_botorch_warnings=suppress_botorch_warnings, verbose_ok_detail=verbose_ok_detail)
            run_jupyter_optimize_all_acquisitions_check(kind=current_kind, cat=True, n=n, d=d, m=m, n_components=n_components, num_epochs=num_epochs, q=q, continue_on_error=continue_on_error, suppress_botorch_warnings=suppress_botorch_warnings, verbose_ok_detail=verbose_ok_detail)
            run_jupyter_optimizer_constraint_support_check(kind=current_kind, cat=False, n=n, d=d, m=m, n_components=n_components, num_epochs=num_epochs, q=q, full_matrix=full_matrix, continue_on_error=continue_on_error, verbose_ok_detail=verbose_ok_detail, verbose_candidates=verbose_candidates, verbose_constraints=verbose_constraints, suppress_botorch_warnings=suppress_botorch_warnings)
            run_jupyter_optimizer_constraint_support_check(kind=current_kind, cat=True, n=n, d=d, m=m, n_components=n_components, num_epochs=num_epochs, q=q, full_matrix=full_matrix, continue_on_error=continue_on_error, verbose_ok_detail=verbose_ok_detail, verbose_candidates=verbose_candidates, verbose_constraints=verbose_constraints, suppress_botorch_warnings=suppress_botorch_warnings)

    print(f"all {kind} projected multi-output binary Jupyter checks passed.")
