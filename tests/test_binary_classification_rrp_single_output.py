...
- fit / inner inputs are transformed-space,
- the sparse outlier likelihood is dimensionally consistent, and
- relevance-pursuit support / sparse parameters are updated consistently.
"""
...
def _build_input_transform(
    ...
    """RRP / mixed RRP 共通の raw-space input_transform を作る。"""
...
def _fit_rrp_binary_model(
    ...
    *,
    ...
    sparsity_upper = min(2, int(model.likelihood.dim))
    sparsity_levels = [0] if sparsity_upper == 0 else [0, sparsity_upper]

    mll = model.make_mll()
    fit_mll, sparse_module, model_trace = fit_rrp_binary_classifier_mll(
        mll,
        method="forward",
    ...
    # テスト側で RRP fit の結果を明示的に確認できるように保持する。
    model._rrp_fit_mll = fit_mll
    model._rrp_sparse_module = sparse_module
    model._rrp_model_trace = model_trace
    model._rrp_sparsity_levels = sparsity_levels


def _assert_rrp_likelihood_state(model: Any, train_x: torch.Tensor) -> None:
    """SparseOutlierBernoulliLikelihood の RRP 固有状態を確認する。"""
    likelihood = model.likelihood
    support = list(getattr(likelihood, "support", []))
    max_sparsity = max(getattr(model, "_rrp_sparsity_levels", [len(support)]))

    assert likelihood.dim == train_x.shape[-2]
    assert len(support) <= max_sparsity
    assert likelihood.raw_delta.shape == torch.Size([len(support)])
    assert likelihood.sparse_parameter.shape == torch.Size([len(support)])
    assert likelihood.dense_delta.shape == torch.Size([train_x.shape[-2]])
    assert torch.isfinite(likelihood.dense_delta).all()

    if hasattr(model, "_rrp_sparse_module"):
        assert model._rrp_sparse_module is likelihood
    if hasattr(model, "_rrp_fit_mll"):
        assert model._rrp_fit_mll.model is model.model
        assert model._rrp_fit_mll.likelihood is likelihood
    if hasattr(model, "_rrp_model_trace"):
        assert model._rrp_model_trace is not None

    expanded_base_indices = getattr(likelihood, "expanded_base_indices", None)
    assert expanded_base_indices is not None
    if model.fit_train_input.shape[-2] == train_x.shape[-2]:
        assert expanded_base_indices.numel() == 0
    else:
        assert expanded_base_indices.numel() == model.fit_train_input.shape[-2]
        assert expanded_base_indices.min().item() >= 0
        assert expanded_base_indices.max().item() < train_x.shape[-2]
...
def _assert_rrp_model_training(
    ...
    *,
    ...
    """Outlier RRP binary classification wrapper の基本状態を確認する。"""
    model.eval()
    ...
    with torch.no_grad():
        ...
        if isinstance(expected_fit_x, tuple):
            expected_fit_x = expected_fit_x[0]
        transformed_x = model.transform_inputs(train_x)
        posterior = model.posterior(train_x)
        latent_posterior = model.latent_posterior(train_x)

    assert transformed_x.shape == expected_fit_x.shape
    assert torch.allclose(transformed_x, expected_fit_x)
    assert model.fit_train_inputs[0].shape == expected_fit_x.shape
    assert torch.allclose(model.fit_train_inputs[0], expected_fit_x)
    assert model.model.train_inputs[0].shape == expected_fit_x.shape
    assert torch.allclose(model.model.train_inputs[0], expected_fit_x)
    assert torch.allclose(model.fit_train_targets, train_y.reshape(-1))
    assert torch.allclose(model.model.train_targets, model.fit_train_targets)
    ...
    assert mll.num_data == model.fit_train_input.shape[-2]

    _assert_rrp_likelihood_state(model, train_x)
...
def create_binary_rrp_model_bundle(
    *,
    ...
    """Jupyter / pytest 共通で使う binary RRP model 作成関数。"""
...
def binary_rrp_model_bundle() -> dict[str, Any]:
    """pytest 用: 通常 binary RRP model。"""
    return create_binary_rrp_model_bundle(
        cat=False,
        n=16,
        d=5,
        num_epochs=8,
...
def binary_rrp_mixed_model_bundle() -> dict[str, Any]:
    """pytest 用: mixed binary RRP model。"""
    return create_binary_rrp_model_bundle(
        cat=True,
        n=16,
        d=5,
...
def _representative_acquisition_cases(
    ...
    """RRP optimizer smoke test 用に代表 acquisition だけを返す。"""
    names = {
        "predictive_entropy",
        "latent_straddle",
        "pof",
...
def _representative_constraint_cases(bounds: torch.Tensor) -> list[dict[str, Any]]:
    """RRP optimizer constraint smoke test 用の軽量ケースだけを返す。"""
...
def _optimizer_constraint_scenarios(
    ...
    *,
    ...
    prefix = "rrp_mixed" if mixed else "rrp"
    return [
        (
            acq_cls,
            kwargs,
...
def test_binary_rrp_model_basic_behavior(
    binary_rrp_model_bundle: dict[str, Any],
) -> None:
    _assert_rrp_model_training(
        model=binary_rrp_model_bundle["model"],
        train_x=binary_rrp_model_bundle["train_x"],
        train_y=binary_rrp_model_bundle["train_y"],
        cat_dims=binary_rrp_model_bundle["cat_dims"],
    )


def test_binary_rrp_mixed_model_basic_behavior(
    binary_rrp_mixed_model_bundle: dict[str, Any],
) -> None:
    _assert_rrp_model_training(
        model=binary_rrp_mixed_model_bundle["model"],
        train_x=binary_rrp_mixed_model_bundle["train_x"],
        train_y=binary_rrp_mixed_model_bundle["train_y"],
        cat_dims=binary_rrp_mixed_model_bundle["cat_dims"],
    )


def test_binary_rrp_acquisition_forward_shapes(
    binary_rrp_model_bundle: dict[str, Any],
) -> None:
    model = binary_rrp_model_bundle["model"]
    train_x = binary_rrp_model_bundle["train_x"]
    X = make_random_batch(
        binary_rrp_model_bundle["bounds"],
        batch_size=4,
        q=2,
    )
...
def test_binary_rrp_mixed_acquisition_forward_shapes(
    binary_rrp_mixed_model_bundle: dict[str, Any],
) -> None:
    model = binary_rrp_mixed_model_bundle["model"]
    train_x = binary_rrp_mixed_model_bundle["train_x"]
    X = make_random_mixed_batch(
        binary_rrp_mixed_model_bundle["bounds"],
        binary_rrp_mixed_model_bundle["cat_dims"],
        batch_size=4,
        q=2,
    )
...
@pytest.mark.slow
def test_binary_rrp_optimize_acqf_representative_smoke(
    binary_rrp_model_bundle: dict[str, Any],
) -> None:
    model = binary_rrp_model_bundle["model"]
    train_x = binary_rrp_model_bundle["train_x"]
    bounds = binary_rrp_model_bundle["bounds"]
    q = 2
...
@pytest.mark.slow
def test_binary_rrp_mixed_optimize_acqf_mixed_representative_smoke(
    binary_rrp_mixed_model_bundle: dict[str, Any],
) -> None:
    model = binary_rrp_mixed_model_bundle["model"]
    train_x = binary_rrp_mixed_model_bundle["train_x"]
    bounds = binary_rrp_mixed_model_bundle["bounds"]
    cat_id = binary_rrp_mixed_model_bundle["cat_dims"][0]
...
@pytest.mark.slow
def test_binary_rrp_optimizer_constraint_case_smoke(
    binary_rrp_model_bundle: dict[str, Any],
) -> None:
    model = binary_rrp_model_bundle["model"]
    train_x = binary_rrp_model_bundle["train_x"]
...
@pytest.mark.slow
def test_binary_rrp_mixed_optimizer_constraint_case_smoke(
    binary_rrp_mixed_model_bundle: dict[str, Any],
) -> None:
    model = binary_rrp_mixed_model_bundle["model"]
    train_x = binary_rrp_mixed_model_bundle["train_x"]
...
def run_jupyter_forward_check(
    *,
    ...
    print("=" * 80)
    print(f"Jupyter RRP forward check cat={cat}")
    if verbose_forward_detail:
        print(f"train_x.shape={train_x.shape}")
        print(f"fit_train_input.shape={model.fit_train_input.shape}")
        print(f"model.train_inputs[0].shape={model.model.train_inputs[0].shape}")
        print(f"likelihood.dim={model.likelihood.dim}")
        print(f"likelihood.support={model.likelihood.support}")
        print(f"likelihood.raw_delta.shape={tuple(model.likelihood.raw_delta.shape)}")
...
def run_jupyter_optimize_acqf_all_acquisitions_check(
    *,
    ...
    print("=" * 100)
    print("Jupyter RRP optimize_acqf check: all acquisitions")
...
def run_jupyter_optimize_acqf_mixed_all_acquisitions_check(
    *,
    ...
    print("=" * 100)
    print("Jupyter RRP optimize_acqf_mixed check: all acquisitions")
    print(
        f"n={n}, d={d}, q={q}, "
...
def run_jupyter_all_checks(
    *,
    ...
    print("all RRP Jupyter checks passed.")
