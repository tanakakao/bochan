...
def _assert_rrp_model_training(
    ...
    *,
    ...
    with torch.no_grad():
        expected_fit_x = model.input_transform(train_x) if model.input_transform is not None else train_x
...
def test_binary_rrp_model_basic_behavior(binary_rrp_model_bundle: dict[str, Any]) -> None:
    _assert_rrp_model_training(
        binary_rrp_model_bundle["model"],
        binary_rrp_model_bundle["train_x"],
        binary_rrp_model_bundle["train_y"],
        cat_dims=binary_rrp_model_bundle["cat_dims"],
    )


def test_binary_rrp_mixed_model_basic_behavior(binary_rrp_mixed_model_bundle: dict[str, Any]) -> None:
    _assert_rrp_model_training(
        binary_rrp_mixed_model_bundle["model"],
        binary_rrp_mixed_model_bundle["train_x"],
        binary_rrp_mixed_model_bundle["train_y"],
...
@pytest.mark.slow
def test_binary_rrp_optimize_acqf_representative_smoke(binary_rrp_model_bundle: dict[str, Any]) -> None:
    model = binary_rrp_model_bundle["model"]
    train_x = binary_rrp_model_bundle["train_x"]
...
@pytest.mark.slow
def test_binary_rrp_optimizer_constraint_case_smoke(binary_rrp_model_bundle: dict[str, Any]) -> None:
    model = binary_rrp_model_bundle["model"]
    train_x = binary_rrp_model_bundle["train_x"]
    bounds = binary_rrp_model_bundle["bounds"]
    q = 2
...
def run_jupyter_forward_check(
    *,
    ...
    bundle = create_binary_rrp_model_bundle(cat=cat, n=n, d=d, num_epochs=num_epochs)
    model = bundle["model"]
...
def run_jupyter_optimize_acqf_all_acquisitions_check(
    *,
    ...
    bundle = create_binary_rrp_model_bundle(cat=False, n=n, d=d, num_epochs=num_epochs)
    model = bundle["model"]
    train_x = bundle["train_x"]
    bounds = bundle["bounds"]
...
def run_jupyter_optimize_acqf_mixed_all_acquisitions_check(
    *,
    ...
    bundle = create_binary_rrp_model_bundle(cat=True, n=n, d=d, num_epochs=num_epochs)
    model = bundle["model"]
    train_x = bundle["train_x"]
    bounds = bundle["bounds"]
