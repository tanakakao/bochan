...
def create_binary_rrp_model_bundle(*, cat: bool = False) -> dict[str, Any]:
    ...
    fit_rrp_binary_classifier_mll(
        ...
        method="backward",
        sparsity_levels=[0, 1],
        optimizer_kwargs={"num_epochs": 20},
    )
...
@pytest.mark.slow
def test_binary_rrp_acquisition_optimize_acqf_smoke(binary_rrp_model_bundle: dict[str, Any]) -> None:
    model = binary_rrp_model_bundle["model"]
    train_x = binary_rrp_model_bundle["train_x"]
...
@pytest.mark.slow
def test_binary_rrp_mixed_acquisition_optimize_acqf_smoke(binary_rrp_mixed_model_bundle: dict[str, Any]) -> None:
    model = binary_rrp_mixed_model_bundle["model"]
    train_x = binary_rrp_mixed_model_bundle["train_x"]
...
@pytest.mark.slow
def test_binary_rrp_optimizer_constraint_compatibility_smoke() -> None:
    bundle = create_binary_rrp_model_bundle(cat=False)
...
@pytest.mark.slow
def test_binary_rrp_mixed_optimizer_constraint_compatibility_smoke() -> None:
    bundle = create_binary_rrp_model_bundle(cat=True)
    model, train_x, bounds = bundle["model"], bundle["train_x"], bundle["bounds"]
    q, d = 2, train_x.shape[-1]
