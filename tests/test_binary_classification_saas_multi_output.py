...
Each output is modeled by an independent single-output SAAS binary classifier
...
def test_saas_multi_output_binary_mixed_acquisition_forward_shapes(saas_multi_output_binary_mixed_model_bundle: dict[str, Any]) -> None:
    ...
    X = make_random_mixed_batch(
        saas_multi_output_binary_mixed_model_bundle["bounds"],
        saas_multi_output_binary_mixed_model_bundle["cat_dims"],
        batch_size=4,
        q=2,
    )
    for acq_cls, kwargs, case_id in multi_output_acquisition_cases(model, train_x):
...
def run_jupyter_forward_check(
    *,
    ...
    bundle = create_saas_multi_output_binary_model_bundle(cat=cat, n=n, d=d, m=m, num_epochs=num_epochs)
    model = bundle["model"]
    ...
    print("=" * 80)
    print(f"Jupyter SAAS multi-output binary forward check cat={cat}")
...
def run_jupyter_all_forward_checks(*, num_epochs: int = 4, verbose_forward_detail: bool = False) -> None:
    ...
    print("all SAAS multi-output binary forward checks passed.")
...
def run_jupyter_optimize_all_acquisitions_check(
    *,
    ...
    print("=" * 100)
    print(f"Jupyter SAAS {prefix}multi-output optimize check: all acquisitions")
...
def run_jupyter_optimizer_constraint_compatibility_check(
    *,
    ...
    print("=" * 100)
    print(f"Jupyter SAAS {prefix}multi-output optimizer / constraint compatibility check")
...
def run_jupyter_all_checks(
    *,
    ...
    """SAAS multi-output binary classification の Jupyter 一括確認 helper。"""
    ...
    print("all SAAS multi-output binary Jupyter checks passed.")
