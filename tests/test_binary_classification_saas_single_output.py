...
assertions.  SAAS-specific checks verify that

- continuous SAAS keeps raw-space public inputs and transformed inner inputs,
- mixed SAAS keeps raw-space public inputs,
- mixed SAAS uses encoded-space internally, and
- the SAAS prior is attached to the latent covariance module.
"""
...
def _build_input_transform(
    ...
    """SAAS / mixed SAAS 共通の raw-space input_transform を作る。"""
...
def _fit_saas_binary_model(
    model: Any,
    *,
    num_epochs: int,
    lr: float = 0.01,
) -> None:
    """SAAS binary classification model の MLL を軽量設定で fit する。"""
...
def _assert_saas_model_training(
    ...
    *,
    ...
    """SAAS wrapper と inner SVGP の基本状態を確認する。"""
    model.eval()
    ...
    if cat_dims:
        ...
        expected_encoded = model.transform_inputs(train_x)
        assert torch.allclose(model.model.train_inputs[0], expected_encoded)
        ...
        for cat_id in cat_dims:
            ...
            assert torch.allclose(one_hot.sum(dim=-1), torch.ones(train_x.shape[0], dtype=train_x.dtype, device=train_x.device))
    else:
        expected_inner_x = (
...
def create_binary_saas_model_bundle(
    *,
    ...
    """Jupyter / pytest 共通で使う binary SAAS model 作成関数。"""
...
def binary_saas_model_bundle() -> dict[str, Any]:
    """pytest 用: 通常 binary SAAS classification model。"""
    return create_binary_saas_model_bundle(
        cat=False,
        n=16,
        d=5,
        num_epochs=8,
...
def binary_saas_mixed_model_bundle() -> dict[str, Any]:
    """pytest 用: mixed binary SAAS classification model。"""
    return create_binary_saas_model_bundle(
        cat=True,
        n=16,
        d=5,
...
def _representative_acquisition_cases(
    ...
    """SAAS optimizer smoke test 用に代表 acquisition だけを返す。"""
    names = {
        "predictive_entropy",
        "latent_straddle",
        "pof",
...
def _representative_constraint_cases(bounds: torch.Tensor) -> list[dict[str, Any]]:
    """SAAS optimizer constraint smoke test 用の軽量ケースだけを返す。"""
...
def test_binary_saas_model_basic_behavior(
    binary_saas_model_bundle: dict[str, Any],
) -> None:
    _assert_saas_model_training(
        model=binary_saas_model_bundle["model"],
        train_x=binary_saas_model_bundle["train_x"],
        train_y=binary_saas_model_bundle["train_y"],
        cat_dims=binary_saas_model_bundle["cat_dims"],
    )


def test_binary_saas_mixed_model_basic_behavior(
    ...
    _assert_saas_model_training(
        ...
        train_y=binary_saas_mixed_model_bundle["train_y"],
        cat_dims=binary_saas_mixed_model_bundle["cat_dims"],
    )


def test_binary_saas_acquisition_forward_shapes(
    binary_saas_model_bundle: dict[str, Any],
) -> None:
    model = binary_saas_model_bundle["model"]
    train_x = binary_saas_model_bundle["train_x"]
    X = make_random_batch(
        binary_saas_model_bundle["bounds"],
        batch_size=4,
        q=2,
    )
...
def test_binary_saas_mixed_acquisition_forward_shapes(
    binary_saas_mixed_model_bundle: dict[str, Any],
) -> None:
    model = binary_saas_mixed_model_bundle["model"]
    train_x = binary_saas_mixed_model_bundle["train_x"]
...
@pytest.mark.slow
def test_binary_saas_optimize_acqf_representative_smoke(
    binary_saas_model_bundle: dict[str, Any],
) -> None:
    model = binary_saas_model_bundle["model"]
    train_x = binary_saas_model_bundle["train_x"]
...
@pytest.mark.slow
def test_binary_saas_optimizer_constraint_case_smoke(
    binary_saas_model_bundle: dict[str, Any],
) -> None:
    model = binary_saas_model_bundle["model"]
    train_x = binary_saas_model_bundle["train_x"]
...
def run_jupyter_forward_check(
    *,
    ...
    bundle = create_binary_saas_model_bundle(
        cat=cat,
        n=n,
        d=d,
        num_epochs=num_epochs,
...
def run_jupyter_all_forward_checks(
    *,
    ...
    print("all SAAS forward checks passed.")
