...
def test_cumulative_probability_projection_is_monotone_and_normalized() -> None:
    raw = np.array([[0.30, 0.70], [0.90, 0.20]])
...
def test_random_forest_ordinal_exposes_probability_ensemble_and_latent_bridge() -> None:
...
def test_ngboost_single_and_ensemble_ordinal_probability_members() -> None:
    ...
    ensemble = NGBoostOrdinalEnsembleModel(
        ...
        train_Y=train_Y,
        estimators=[
            [_FakeNGBoost(intercept=0.25), _FakeNGBoost(intercept=0.55)],
            [_FakeNGBoost(intercept=0.45), _FakeNGBoost(intercept=0.50)],
            [_FakeNGBoost(intercept=0.60), _FakeNGBoost(intercept=0.30)],
        ],
        bootstrap=False,
    ).fit()
...
def test_external_ordinal_works_with_existing_acquisitions() -> None:
    train_X, train_Y = _training_data()
    model = RandomForestOrdinalModel(
        train_X=train_X,
        train_Y=train_Y,
    ...
    expected_utility = qOrdinalExpectedUtility(model=model, objective=objective)(X)
...
def test_default_registry_resolves_external_ordinal_models(model_type, expected_cls) -> None:
...
def test_mixed_ordinal_one_hot_encodes_at_estimator_boundary(model_cls) -> None:
    train_X, train_Y = _mixed_training_data()
    estimators = _rf_estimators() if model_cls is RandomForestMixedOrdinalModel else _ng_estimators()
