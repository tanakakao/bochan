...
def test_hetero_binary_ehvi_uses_default_sampler_in_forward() -> None:
    train_X = torch.tensor([[0.0], [0.5], [1.0]], dtype=torch.double)
    train_Y = torch.tensor(
        [[0.1, 0.9], [0.5, 0.5], [0.9, 0.1]],
        dtype=torch.double,
    ...
    assert acqf.sampler is not None
    assert acqf.sampler.sample_shape == torch.Size([128])

    candidates = torch.tensor([[[0.25], [0.75]]], dtype=torch.double)
