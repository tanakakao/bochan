...
class _DummyNoiseModel(nn.Module):
    """Fast differentiable stand-in for the fitted log-variance GP."""
...
def _make_count_data(n: int = 8) -> tuple[Tensor, Tensor, Tensor]:
    ...
    rate = 1.0 + 2.0 * train_x[:, 0] + train_x[:, 1]
...
def test_poisson_heteroscedastic_default_noise_target_path_initializes(
    ...
    monkeypatch.setattr(
        hetero_module,
        "_estimate_poisson_noise_targets",
...
def test_poisson_heteroscedastic_acquisitions_are_finite_and_differentiable(
...
def test_poisson_mixed_heteroscedastic_registers_noise_model_after_base_init(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    train_x, train_y, train_yvar = _make_count_data()
    ...
    model = HeteroscedasticPoissonMixedGPModel(
        train_X=mixed_x,
        train_Y=train_y,
        train_Yvar=train_yvar,
        cat_dims=[2],
