...
def test_parse_formula_supports_groups_and_hydrates():
    ...
    assert parse_formula("CuSO4·5H2O") == {
        "Cu": 1.0,
        "S": 1.0,
        "O": 9.0,
        "H": 10.0,
...
def test_log_ratio_round_trip(method):
    ...
    recovered = transform.inverse_transform(
...
def test_search_space_repairs_bounds_total_steps_and_sparsity():
    ...
    repaired = space.repair(
        {"Fe": 0.51, "Ni": 0.28, "Co": 0.15, "Cr": 0.06}
    )
...
def test_preprocessor_bridges_existing_tabular_dataframe_api():
    ...
    recovered = preprocessor.inverse_candidates(
        transformed.drop(columns=["property"])
    )
