...
def test_tabular_crabnet_mixed_dkl_fits_predicts_and_optimizes_categories() -> None:
    torch.manual_seed(0)
    ...
    assert isinstance(model, CrabNetMixedDKLModel)
    ...
    assert model.category_cardinalities == (3, 2)
    assert model.category_embedding_dims == (3, 2)
    assert bundle.metadata["fit_func"] == "fit_deepkernel_mll"
    assert all(parameter.requires_grad for parameter in model.category_embeddings.parameters())

    layers = model.material_encoder.encoder.transformer_encoder.layers
    assert not any(parameter.requires_grad for parameter in layers[0].parameters())
...
def test_crabnet_mixed_dkl_uses_global_category_cardinalities_for_cv() -> None:
    torch.manual_seed(0)
    optimizer = _optimizer(cross_validation=True).fit(_frame())

    assert optimizer.cross_validation_result_ is not None
    assert optimizer.bo.bundle is not None
    model = optimizer.bo.bundle.model
    assert isinstance(model, CrabNetMixedDKLModel)
    assert model.category_cardinalities == (3, 2)


def test_crabnet_mixed_dkl_requires_categorical_process_input() -> None:
    ...
    try:
        ...
        raise AssertionError("crabnet_mixed_dkl must reject all-continuous process inputs")
