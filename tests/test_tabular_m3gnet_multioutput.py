...
def test_m3gnet_gp_multi_output_auto_builds_independent_model_list() -> None:
    ...
    assert all(isinstance(model, M3GNetGPModel) for model in bundle.model.models)
...
def test_m3gnet_mixed_multi_output_preserves_process_categories() -> None:
    ...
    assert all(isinstance(model, M3GNetMixedGPModel) for model in bundle.model.models)
...
def test_m3gnet_dkl_multi_output_uses_independent_trainable_encoders() -> None:
    optimizer = _optimizer("m3gnet_dkl").fit(_frame(mixed=False))
    bundle = optimizer.bo.bundle

    assert bundle is not None
    assert isinstance(bundle.model, ModelListGP)
    first, second = bundle.model.models
    assert isinstance(first, M3GNetDKLModel)
    assert isinstance(second, M3GNetDKLModel)
...
def test_m3gnet_mixed_dkl_multi_output_uses_mixed_submodels() -> None:
    optimizer = _optimizer("m3gnet_dkl", mixed=True).fit(_frame(mixed=True))
    bundle = optimizer.bo.bundle

    assert bundle is not None
    assert isinstance(bundle.model, ModelListGP)
    assert all(isinstance(model, M3GNetMixedDKLModel) for model in bundle.model.models)
    assert all(model.cat_dims == [2] for model in bundle.model.models)


def test_m3gnet_multi_output_candidate_keeps_structure_and_category_contract(
...
def test_m3gnet_rejects_explicit_user_multi_output_config() -> None:
    with pytest.raises(
        ValueError,
        match="derives independent multi-output structure automatically",
    ):
        TabularBayesianOptimizer(
...
def test_m3gnet_multi_output_array_fit_derives_default_output_names() -> None:
    X = [
        [0.0, 900.0, 0.8],
        [1.0, 950.0, 1.0],
        [2.0, 1000.0, 1.2],
